...
def run():
    ...
    # viewrange for y-axis should be [-2,2]
    ...
    for i in range(len(gPoints)):
        ...
        ax.text(gPoints[i][0], gPoints[i][1], gPoints[i][2], gPoints[i]
                [3], fontsize=9, bbox=dict(facecolor='yellow', alpha=0.5))
        if not i == 0:
            ax.plot([gPoints[(i-1)][0], gPoints[i][0]], [gPoints[(i-1)]
                    [1], gPoints[i][1]], [gPoints[(i-1)][2], gPoints[i][2]], c)
